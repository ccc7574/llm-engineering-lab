...
from stage_coding.agentic_dataset import load_agentic_coding_tasks
from stage_coding.agentic_runner import build_run
from stage_coding.execution import evaluate_python_patch


@dataclass
class AgenticCodingEvalResult:
    task_id: str
    strategy: str
    passed: bool
...
def main() -> None:
    ...
    for task in tasks:
        ...
        result = AgenticCodingEvalResult(
        ...
        print(
            ...
            f"patch_attempts={result.patch_attempts} repair_attempts={result.repair_attempts}"
        )
        print(
    ...
    repaired_successes = sum(1 for result in results if result.passed and result.repair_attempts > 0)
    summary = {
        "strategy": args.strategy,
        "task_success_rate": passed / total,
        "passed": passed,
        "total": len(results),
        "recovery_success_rate": repaired_successes / total,
        ...
        "avg_repair_attempts": sum(result.repair_attempts for result in results) / total,
    ...
    print(f"avg_repair_attempts={summary['avg_repair_attempts']:.2f}")
