...
- `stage3_reasoning` / `stage5_toy_alignment` 已补到 rejection sampling、toy DPO / GRPO-like、alignment eval 与结果解释
- `pretraining` 已补到 `P10/P11` runnable：data mixture 和 continued pretraining 不再只停留在任务文档
- `coding` 的更真实 repo packing、judge / verifier、多文件修复、agentic coding loop、SWE-bench 风格 triage 与 `pass@k` sampling
- `multimodal` 的 OCR 噪声、表格抽取、grounding 和 document workflow 级跨页决策管线
- `multimodal` 已进一步补到 route-policy SFT，可对照 `heuristic_router` 与 `learned_router` 的 dispatch 质量差异
- `agentic` 的更长链路、多任务记忆、reflection、planner-observer 与更复杂 recovery
- `harness` 的更完整回归门槛、成本侧指标、suite automation、changed-scope CI launcher、notification digest、external payload、dispatch、routing policy、override、failure-category-aware route matching、route matrix、route diff、policy gate、workflow gate、dispatch policy、review summary、trend board、retry / idempotency、provider-aware ack / backoff、live adapter security、release note、PR comment writeback、PR comment diagnostics 和统一发布视图
- `harness` 已进一步补到 artifact catalog、baseline snapshot 和细粒度 failure replay plan，开始具备最小 artifact 治理能力
- `harness` 已进一步补到 notification delivery orchestration，route / dispatch policy / live dispatch 不再依赖 workflow 里手工拼接
...
python3 code/stage1_nanogpt_core/train_mixture.py --manifest-path datasets/tiny_pretraining_mixture/uniform.json --out-dir runs/stage1_mixture_uniform --max-iters 80 --eval-interval 40
python3 code/stage1_nanogpt_core/continue_pretraining.py --checkpoint runs/stage1_gpt/ckpt.pt --data-path datasets/tiny_pretraining_payments/input.txt --eval-set general=datasets/tiny_pretraining_general/input.txt --eval-set domain=datasets/tiny_pretraining_payments/input.txt --out-dir runs/stage1_payments_adapted --max-iters 80 --eval-interval 40
...
python3 code/stage_multimodal/train_multimodal_sft.py --data-path datasets/tiny_multimodal_sft/train.jsonl --out-dir runs/multimodal_sft_router --max-iters 200 --eval-interval 50 --learning-rate 0.2
python3 eval/multimodal_sft_eval.py --data-path datasets/tiny_multimodal_sft/eval.jsonl --strategy learned_router --checkpoint runs/multimodal_sft_router/router.pt --report-path runs/multimodal_sft_learned_router.json
...
CI / 回归套件的中文说明见 [ci_regression_guide_zh.md](/Volumes/ExtaData/newcode/llm-engineering-lab/docs/ci_regression_guide_zh.md)。
