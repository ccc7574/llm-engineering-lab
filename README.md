...
- `harness` 的更完整回归门槛、成本侧指标、suite automation、changed-scope CI launcher、notification digest、external payload、dispatch、routing policy、override、failure-category-aware route matching、route matrix、route diff、policy gate、workflow gate、dispatch policy、review summary、trend board、retry / idempotency、provider-aware ack / backoff、release note、PR comment writeback、PR comment diagnostics 和统一发布视图
