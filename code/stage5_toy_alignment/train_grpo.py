...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", default="runs/stage5_grpo")
...
def main() -> None:
    ...
    print(
        ...
        f"block_size={model_config.block_size} kl_coef={args.kl_coef}"
    )

    for step in range(args.max_iters):
        if step % args.eval_interval == 0 or step == args.max_iters - 1:
            metrics = evaluate_preference_pairs(policy_model, reference_model, pairs, device, beta=0.1)
        ...
        reward_chosen = torch.ones_like(policy_chosen)
        reward_rejected = -torch.ones_like(policy_rejected)
        stacked_rewards = torch.stack([reward_chosen, reward_rejected], dim=1)
        advantages = stacked_rewards - stacked_rewards.mean(dim=1, keepdim=True)
        policy_objective = -(
            advantages[:, 0] * policy_chosen + advantages[:, 1] * policy_rejected
        ).mean()
        kl_penalty = ((policy_chosen - ref_chosen) ** 2 + (policy_rejected - ref_rejected) ** 2).mean()
        loss = policy_objective + args.kl_coef * kl_penalty

        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()

    final_metrics = evaluate_preference_pairs(policy_model, reference_model, pairs, device, beta=0.1)
